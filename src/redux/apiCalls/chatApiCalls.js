...
export function accessChatId(userId) {
    return async (dispatch, getState) => {
        try {
            ...
            const { data } = await axios.post(`https://backend-chat-6wma.onrender.com/api/v1/chats`, userId, {
                headers: {
...
export function fetchChat(user) {
    return async (dispatch, getState) => {
        try {
            dispatch(chatActions.setLoading())
            const { data } = await axios.get(`https://backend-chat-6wma.onrender.com/api/v1/chats`, {
                headers: {
                    Authorization: "Bearer " + getState().auth.user.token,
                }
            })
